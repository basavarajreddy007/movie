...
import { Bookmark, Menu, X, UserIcon, LogOutIcon } from "./icons";
      ...
      <div className="w-full max-w-7xl mx-auto px-4 sm:px-6 py-2.5 sm:py-0 min-h-[64px] sm:min-h-[72px] flex flex-wrap md:flex-nowrap items-center justify-between gap-3 md:gap-6 relative">
        ...
        <div className="order-2 md:order-3 hidden md:flex items-center gap-3 flex-shrink-0">
          <Link
            ...
            title="Bookmarks"
            aria-label={`View bookmarks (${isAuthenticated ? bookmarkCount : 0})`}
          >
            <Bookmark size={18} />
            <span>Bookmarks</span>
          ...
          {isAuthenticated && user ? (
            <div className="relative" ref={userMenuRef}>
              ...
              >
                <div className="w-7 h-7 rounded-lg bg-gradient-to-tr from-[#FF3D68] to-[#FFA06B] flex items-center justify-center text-white font-bold text-xs shadow-sm">
              ...
              {isUserMenuOpen && (
                <div className="absolute right-0 mt-2 w-56 rounded-2xl bg-white dark:bg-[#0F1322] border border-slate-200 dark:border-white/10 shadow-2xl p-2 z-50 animate-fadeIn">
        ...
        <button
          type="button"
          className="order-2 md:hidden flex items-center justify-center w-10 h-10 rounded-xl border border-slate-200 dark:border-white/10 bg-white dark:bg-[#121625] text-slate-700 dark:text-slate-200 hover:bg-[#FF3D68] hover:border-[#FF3D68] hover:text-white transition-colors cursor-pointer"
          ...
          {isAuthenticated && user ? (
            <div className="p-2.5 rounded-xl border border-slate-200 dark:border-white/10 bg-slate-50 dark:bg-[#121625] flex items-center justify-between">
              <div className="flex items-center gap-2 min-w-0">
                <div className="w-8 h-8 rounded-lg bg-gradient-to-tr from-[#FF3D68] to-[#FFA06B] flex items-center justify-center text-white font-bold text-xs flex-shrink-0">
                  {user.name.charAt(0).toUpperCase()}
                </div>
